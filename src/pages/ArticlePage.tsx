...
import { ArrowLeft, Share2, Calendar, Eye, MessageCircle } from 'lucide-react';
...
export function ArticlePage({ slug, onNavigateBack, onNavigateToAuthor }: ArticlePageProps) {
  ...
  const handleAddComment = async (postId: string, content: string) => {
    ...
    // You could update the local state here to optimistically update the UI
  };
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground flex-wrap">
            <div className="flex items-center gap-1">
              ...
              <span className="hidden sm:inline">{post.viewCount.toLocaleString()} vues</span>
              <span className="sm:hidden">{post.viewCount.toLocaleString()}</span>
            </div>
            <div className="flex items-center gap-1">
              <MessageCircle className="h-4 w-4" />
              <span className="hidden sm:inline">{post.commentCount} commentaires</span>
              <span className="sm:hidden">{post.commentCount}</span>
            </div>
            <div className="flex items-center gap-1">
              <Calendar className="h-4 w-4" />
              <span className="hidden sm:inline">{formatDate(post.createdAt)}</span>
              <span className="sm:hidden">{new Date(post.createdAt).toLocaleDateString('fr-FR')}</span>
            </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-8">
      ...
      {/* Comments Section */}
      <ProtectedGate showPreview={false}>
        <div className="mx-auto max-w-4xl">
          <CommentsSection 
